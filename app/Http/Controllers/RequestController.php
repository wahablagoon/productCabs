...
class RequestController extends Controller
{
	...
	{		
		...
		$res_driver_all = $this->Drivers_model->find_data('drivers',array('_id'=>new MongoId($doc['_id']->{'$id'})));  // getting driver data for ride share
		 foreach($res_driver_all as $res_driver){}
		 
		if(isset($res_driver['used_share']))
		$used_share = $res_driver['used_share'] ;
		else
		$used_share = 0 ;	
		
		if(isset($res_driver['max_share']))
		$max_share_rider = $res_driver['max_share'] ;
		else
		$max_share_rider = 1 ;
				if(($ride_type == "shared" && $used_share < $max_share_rider && $max_share_rider > 0) || (($current_status['request']['status'] == 0 || $current_status['request']['status'] == "")  &&  ($current_status['accept']['status'] == 0 ||  $current_status['accept']['status'] == '')))
				{
				// Check driver data 
				$get_dist = GetDrivingDistance($doc['lat'],$doc['long'],$getRequestData['pickup']['lat'] ,$getRequestData['pickup']['long'] );
		     
			 // echo "test".$request_id ; 
			  if($fareEST === NULL){$fareEST="0";}
			  
				$firebase->update(array(
				'request/status' => 1,
				'request/eta' => $get_dist['time'],
				'request/distance' => $get_dist['distance'],
				'request/estFare' => $fareEST,
				'request/pickupAddress' => $pickup_address,
				'request/dropAddress' => $drop_address,
				'request/req_id' => $request_id,
				'request/rider_id' => $rider_id_fb
				),'drivers_data/'.$doc['_id']->{'$id'}); // Update driver req status in fb

				}
				
				
					for($timer=0; $timer<$request_duration_time; $timer++) {
						$current_status = $firebase->get('drivers_data/'.$doc['_id']->{'$id'});
   						if ( $current_status['accept']['status'] != 1 ) {
      						  sleep(1);
   							 }
						else {
						break;	
				...
				// insert & upddate the shared riders count in driver table 
				if($ride_type == "shared")
				{
					//echo "new entry";
					if(isset($res_driver['is_share']) && $res_driver['is_share'] == "yes")
					{
					$updatekey = array("_id"=> new MongoId($doc['_id']->{'$id'}));
					$update_data_req['is_share'] = "yes";
					$update_data_req['used_share'] = $res_driver['used_share'] + 1;
					$this->Drivers_model->update_driver($updatekey, $update_data_req); 			
					}else{
					$updatekey = array("_id"=> new MongoId($doc['_id']->{'$id'}));
					$update_data_req['is_share'] = "yes";
					$update_data_req['used_share'] = 1;
					$update_data_req['max_share'] = $max_share;
					$this->Drivers_model->update_driver($updatekey, $update_data_req); 
					}
				}else{
					$updatekey = array("_id"=> new MongoId($doc['_id']->{'$id'}));
					$update_data_req['is_share'] = "no";
					$this->Drivers_model->update_driver($updatekey, $update_data_req); 	
				}				
				//// end
				...
				if(isset($getRequestData['request_type']))
				{
					$data_trip['request_type']= $getRequestData['request_type'];
				}else{
					$data_trip['request_type']= "normal";					
				}
				//$get_dist = GetDrivingDistance($doc['lat'],$doc['long'],$getRequestData['pickup']['lat'] ,$getRequestData['pickup']['long'] );
				
				//$data_trip['eta'] = $get_dist['time'];
				$data_trip['total_price'] = 0 ;
				$data_trip['admin_commission'] = 0 ;
				...
				$res_corp = $this->mongo_db->db->riders->find(array('_id'=>new MongoId($data_trip['rider_id'])));
				
				$driver_res_corp = $this->mongo_db->db->drivers->find(array('_id'=>new MongoId($data_trip['driver_id'])));
				...
				if($res_corp->count()>0)
				{
	                 ...
	                 if(isset($row_corp['c_id']) && $row_corp['c_id'] != null && $row_corp['c_id'] != "")
					 {
					 $data_trip['c_id'] = $row_corp['c_id'] ;
					 $data_trip['coporate_payment_status'] = 'Not completed' ;
					 }else{
					 $data_trip['c_id'] = '' ;	
					 }
				$rider_name = $row_corp['first_name'];
				$rider_email = $row_corp['email'];
				}else{
				$data_trip['c_id'] = '' ;
				$rider_name = 'Not set';
				$rider_email = 'Not set';
				...
				if($driver_res_corp->count()>0)
				{
										
					 foreach($driver_res_corp as $driver_row_corp){
					 	$driver_name = $driver_row_corp['first_name'];
					 	$driver_mobile = $driver_row_corp['mobile'];
						if(isset($driver_row_corp['company_id']))
						{
							$data_trip['company_id'] = $driver_row_corp['company_id'] ;	
						}
						else
						{
							$data_trip['company_id'] = '';	
						}
					 }
				}else{
				$driver_name = 'Not set' ;
				$driver_mobile = 'Not set';
				}	
				...
				unset($data_trip['_id']);
				 ...
				 $firebase->update(array(
						 'accept/trip_id' => $data_trip['trip_id'],
						  'accept/trip_id_rider_name' => $data_trip['trip_id'].":".$rider_name
				 ...
				 $response_array = $data_trip ;
						...
						$email = $rider_email;
						$splvars = array("{rider_name}"=> $rider_name ,"{driver_name}"=>$driver_name, "{driver_phone}"=>$driver_mobile, "{datetime}"=>date('m/d/Y',$getRequestData['created']));
					//	$code = "Accept";
